...
call fromelf --vhx --32x1 --output bram_a7.hex objects\hello_world.axf
@REM - File to merge SW into bitstream
@REM axf file will work, but has to be renamed to .elf for updatemem to work
copy  objects\hello_world.axf bram_a7.elf
@REM - File to load into DAPLink QSPI simulation
call fromelf --vhx --8x1  --output qspi_a7.hex objects\hello_world.axf
@REM - Files to load onto DAPLink QSPI board
call fromelf --bin        --output qspi_a7.bin objects\hello_world.axf
